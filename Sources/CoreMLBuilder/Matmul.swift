...
public struct Matmul {
    ...
    private func modelDescription() -> ModelDescription {
        ModelDescription(
            input: (x != nil ? [] : [
                FeatureDescription(
                    name: "x",
                    type: FeatureType(multiArray: [Int64(xShape.0), Int64(xShape.1)], dataType: arrayDataType)
                ),
            ]) + (y != nil ? [] : [
                FeatureDescription(
                    name: "y",
                    type: FeatureType(multiArray: [Int64(yShape.0), Int64(yShape.1)], dataType: arrayDataType)
                )
            ]),
            output: [
                FeatureDescription(
        ...
        return MILSpec_Program(
            ...
            functions: ["main": MILSpec_Function(
                ...
                blockSpecializations: ["CoreML6": MILSpec_Block(
                    outputs: ["output"],
                    operations: (
                        declareInput("x", shape: xShape, data: x) +
    ...
    private func declareInput(_ name: String, shape: (Int64, Int64), data: TensorData?) -> [MILSpec_Operation] {
        guard let data = data else {
            return []
        }
        return [MILSpec_Operation(
            constWithName: name,
